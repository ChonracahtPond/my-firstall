                ...
                <div>
                    {markData.slice(0, visibleItems).map((mark, index) => (
                        <div key={index} className="mb-4">
                            <div className={`border border-t-0 border-neutral-200 bg-[#E3FFD2] dark:border-neutral-600 dark:bg-neutral-800 ${expandedItemIndex === index ? 'rounded-t' : 'rounded'}`}>
                                <h2 className="mb-0" id={`heading${index}`}>
                                    <button
                                        className={`group relative flex w-full items-center rounded-none border-0 bg-white px-5 py-4 text-left text-base text-neutral-800 transition ${expandedItemIndex === index ? 'bg-white text-primary' : ''} hover:z-[2] focus:z-[3] focus:outline-none dark:bg-neutral-800 dark:text-white`}
                                        type="button"
